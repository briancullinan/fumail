...
(function($){		
	$.fn.jqte = function(options){
		...
		$.fn.jqteVal = function(value){
            var jqte = $(this).closest("."+vars.css),
                editor = jqte.find("."+vars.css+"_editor"),
                placeholder = jqte.find("."+vars.css+"_placeholder");
            editor.html(value);
            if(placeholder.length > 0)
                editor.text()!="" ? placeholder.hide() : placeholder.show();
		}
		...
		return this.each(function(){
			...
			{
				jQTE.prepend('<div class="'+vars.css+'_placeholder" style="display:block" unselectable><div class="'+vars.css+'_placeholder_text">'+vars.placeholder+'</div></div>');
